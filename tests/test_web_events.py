...
def test_approval_requires_the_exact_waiting_hash_and_starts_one_continuation(
    ...
    wrong = asyncio.run(
        _request(app, "POST", f"/tasks/{task.id}/approval", data={"action_hash": "wrong", "decision": "approve"})
    )
    assert wrong.status_code == 409
    assert target.exists()
    accepted = asyncio.run(
        _request(app, "POST", f"/tasks/{task.id}/approval", data={"action_hash": action_hash, "decision": "approve"})
    )
    replay = asyncio.run(
        _request(app, "POST", f"/tasks/{task.id}/approval", data={"action_hash": action_hash, "decision": "approve"})
    )
...
def test_unknown_task_resources_return_not_found(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    approval = asyncio.run(_request(app, "POST", f"/tasks/{unknown}/approval", data={"action_hash": "x", "decision": "approve"}))
